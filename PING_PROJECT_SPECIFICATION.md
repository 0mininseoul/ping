...
- 한 핑에 여러 명이 회신하면 **모았다가 한 번에** 띄운다. 룸의 본인 제외 인원이 다 모이면 즉시, 아니면 첫 회신이 준비된 때로부터 5초 뒤에 함께 페이드인한다. 1:1 룸은 기다리지 않아 지연이 없다. 묶음 결과는 `auto_reply_batch_presented`(collected / expected / timed_out)로 남긴다.
- 회신은 원본 핑의 좌표를 그대로 물고 오므로 창을 하나씩 놓으면 완전히 포개진다. 함께 뜨는 창은 원본 핑 자리를 중심으로 한 줄에 3개까지 격자로 배치하고, 화면 밖으로 나가지 않게 클램프한다.
