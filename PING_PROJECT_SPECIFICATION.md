# Ping — 실시간 3초 영상 메시지 macOS/Windows 앱 기획서 (v2.7)
...
- 프리뷰가 준비되면 `Option+스크롤`, 두 손가락 핀치, `Option+커서 이동`, `Enter`, `Esc` 사용법을 3초간 상세히 안내한 뒤, 화면을 덜 가리는 한 줄 축약 안내로 자동 전환한다.
- `Option+아래에서 위로 스크롤`하면 화면 캡처 영역을 확대하고, `Option+위에서 아래로 스크롤`하면 축소한다.
- 트랙패드에서 두 손가락을 펼치면 확대하고, 오므리면 축소한다. 핀치는 Option 키 없이 동작한다.
