...
- **맥이 잠든 사이 받은 핑에 자동 회신이 뒤늦게 나가던 문제를 고쳤다.** 화면이 꺼진 채 반쯤 깨어 있는 동안(DarkWake)에도 핑은 실시간으로 도착하는데, 카메라는 돌지 않아 녹화가 멈춘 채 기다리다 맥을 깨우는 순간 얼굴을 찍어 보냈다(3시간 넘게 늦은 사례). 이제 화면이 꺼져 있으면 자동 회신을 시작하지 않고, 녹화 직전과 전송 직전에 다시 확인해 60초가 지났으면 보내지 않으며, 녹화 도중 잠들면 카메라를 끊는다.
...
- **핑을 실시간으로 받으면 자동으로 얼굴을 3초 녹화해 보낸 사람에게 되돌려 보낸다.** 설정 › 일반 › "핑 받으면 자동으로 얼굴 회신"으로 끄고 켤 수 있고 기본값은 켜짐이다. 녹화하는 동안 화면 우상단에 표시가 뜬다(카메라 화면은 띄우지 않는다). 자동 회신으로 온 핑에는 다시 자동 회신하지 않고, 맥이 꺼져 있었거나 도착한 지 1분이 지난 핑, 거울을 쓰는 중이면 건너뛴다.
...
- **실시간 연결이 처음으로 실제 동작한다.** 지금까지 Realtime 소켓 주소를 만들 때 `URLComponents`를 썼는데, `Supabase.plist`의 프로젝트 URL이 슬래시로 끝나지 않아 경로가 `realtime/v1`(앞 슬래시 없음)이 됐고 이 조합에서는 URL 생성이 nil을 돌려준다. 그래서 연결을 **시도조차 못 하고** 매번 10초 폴링으로 떨어졌다. 채팅과 핑이 느렸던 근본 원인이다.
- **영상 핑이 도착 즉시 뜬다.** 영상은 실시간 구독 대상이 아니어서 10초 폴링으로만 왔고 체감 7초쯤 걸렸다. 이제 나에게 오는 영상 메시지를 실시간으로 감지해 바로 가져온다. 폴링은 그대로 두어 연결이 끊겨도 놓치지 않는다.
- **채팅 알림이 배너로 다시 뜬다.** 폴백 폴링이 앱을 켤 때마다 최근 채팅 20건을 "새 메시지"로 재생해 한 번에 21건을 발송했고, macOS가 그 폭주를 배너 없이 알림 센터로 흘려보냈다. 이제 폴링 시작 이후에 도착한 메시지만 알린다. 밀려 있던 채팅은 기존대로 룸당 1건으로 묶어 알린다.
...
- 상대가 보낸 영상이 알림 클릭 없이 화면에 바로 뜬다. 발신자가 지정한 위치에 그대로 재생되고, 재생이 끝나면 10초 뒤 저절로 사라진다. 작업 중인 앱의 포커스는 뺏지 않는다. 설정 → 일반 → "받은 영상 바로 재생"에서 끌 수 있다. 앱을 켜기 전에 밀려 있던 핑은 창이 여러 개 겹치지 않도록 알림으로만 알린다.
- 영상 수신 알림이 2026-06-08부터 전혀 뜨지 않던 문제를 고쳤다. 알림 직전에 영상을 미리 받아 두는 코드가 자기 자신의 다운로드를 기다리며 멈췄고, 수신 처리가 직렬이라 첫 영상에서 멈추면 그 뒤 모든 영상 알림이 함께 죽었다. 이제 알림을 먼저 보내고 재생 준비는 따로 진행해서, 느린 다운로드 한 건이 뒤따르는 알림을 막지 못한다.
