...
## macOS APNs 알림과 수신 재생

macOS의 영상, 채팅, 룸 초대 배너는 Supabase polling이나 macOS 로컬 예약이 아니라 APNs로만 전달된다. Realtime과 polling은 룸 상태·히스토리 갱신과 실행 중인 Mac의 자동 얼굴 회신에만 사용한다. Sparkle 업데이트 알림처럼 기기에서 시작되는 업데이트 안내는 기존 로컬 알림으로 남는다.

수신자별 APNs 라우팅은 다음과 같이 배타적이다. `live Mac presence`는 `desktop_presence`의 `ended_at`이 비어 있고 기존 45초 TTL 안에 갱신된 행이다.
...
자동 얼굴 회신은 이미 실행 중인 macOS 프로세스가 새 영상 이벤트를 실시간으로 받을 때만, 원 발신자 한 명에게 3초 영상을 보낸다. 앱 시작 전 행, APNs cold-start 클릭으로 불러온 행, polling catch-up 행에는 자동 회신하지 않는다.

룸 히스토리에서 화면+얼굴 영상을 클릭하면 룸 창 안 overlay가 아닌 별도 floating 플레이어가 열린다. 목표 content width는 600pt이며 저장된 aspect ratio를 유지하고, 화면 가장자리 32pt 여백 안에 맞지 않으면 비례 축소한다. 얼굴 전용 200pt 원형 재생은 그대로다.

백엔드의 APNs 키·topic·webhook 설정과 production smoke 절차는 [`docs/PUSH_BACKEND_SETUP.md`](docs/PUSH_BACKEND_SETUP.md)에 정리되어 있다.

## 설치

### macOS

1. `Ping-v0.3.79.dmg`를 더블클릭해 마운트한다.
...
- 수신 재생창에 `Esc 닫기` 안내를 표시해 재생 중에도 창을 닫는 단축키를 바로 알 수 있게 했다.

## v0.3.78 macOS

- macOS 영상·채팅·룸 초대 알림을 APNs 원격 알림으로 전달하고, Release 빌드의 production APNs entitlement와 Developer ID provisioning profile을 적용했다.
- 화면+얼굴 히스토리 재생을 별도 floating 플레이어로 열고 목표 content width 600pt를 유지한다. 얼굴 전용 재생은 200pt 원형 창으로 유지한다.

## v0.3.77 macOS

- **맥이 잠든 사이 받은 핑에 자동 회신이 뒤늦게 나가던 문제를 고쳤다.** 자동 회신은 이미 실행 중인 Mac이 실시간으로 받은 fresh 영상에만 적용한다. 화면이 꺼져 있으면 시작하지 않고, 녹화 직전과 전송 직전에 다시 확인해 60초가 지났으면 보내지 않으며, 녹화 도중 잠들면 카메라를 끊는다.
- **내가 보낸 영상과 메시지는 보낸 뒤 5분 안에만 우클릭 → 삭제로 모두에게서 지울 수 있다.** 5분이 지나면 삭제 메뉴가 사라지고, 서버도 같은 기준으로 거절한다(구버전 앱 포함). 받은 영상을 내 목록에서 숨기는 삭제는 그대로다.

## v0.3.76 macOS

- **창이 두 개 열리던 문제를 실제로 고쳤다.** 0.3.75의 수정은 듣지 않았다. launchd가 소유권을 넘겨받을 때 이전 인스턴스에 종료를 "요청"하는 방식이었는데, Ping은 샌드박스 앱이라 그 요청(quit AppleEvent)을 보낼 권한이 없어 전달되지 않았다. 이제 방향을 뒤집어, 등록을 한 쪽이 새 인스턴스가 뜬 것을 확인하면 **스스로** 물러난다. 남의 프로세스를 끝내는 데 의존하지 않는다.
...
- 자동 얼굴 회신은 이미 실행 중인 Mac이 fresh 영상 이벤트를 받을 때만 적용된다. 설정에서 끌 수 없고 관련 선택 UI도 표시하지 않는다. 자동 회신 루프 차단, 60초 freshness 제한, 카메라 상태 확인, 녹화 인디케이터는 유지한다.
...
- **알림이 오지 않는데 시스템 설정에서 켤 수조차 없던 문제를 고쳤다.** Ping은 알림 권한을 최초 설정 화면에서만 요청했는데, 계정이 이미 있는 상태로 새로 설치하거나 다른 기기에서 세션을 넘겨받으면 그 화면이 아예 뜨지 않는다. 그러면 권한을 한 번도 요청하지 않은 상태로 남고, macOS는 그런 앱을 **시스템 설정 › 알림 목록에 표시조차 하지 않는다.** 알림도 오지 않고 켤 방법도 없는 상태였고, 업데이트해도 어느 경로에서도 권한을 묻지 않아 그대로였다. 이제 그런 기기는 실행할 때 한 번 권한을 요청해 목록에 등록되고, 설정 › 일반에 "알림 권한" 항목이 생겨 언제든 상태를 확인하고 다시 켤 수 있다.
- **핑을 실시간으로 받으면 자동으로 얼굴을 3초 녹화해 보낸 사람에게 되돌려 보낸다.** 이미 실행 중인 Mac이 받은 fresh 영상에만 적용하며, 앱 시작 전 행이나 APNs cold-start/polling catch-up에는 자동 회신하지 않는다. 녹화하는 동안 화면 우상단에 표시가 뜨고(카메라 화면은 띄우지 않는다), 자동 회신으로 온 핑에는 다시 회신하지 않는다.

## v0.3.71 macOS 수정

- **텍스트 메시지 알림이 뜨자마자 사라지던 문제를 찾아 고쳤다.** 룸 창을 열어둔 채 다른 앱을 쓰는 동안 새 채팅이 오면, 알림은 정상적으로 올라갔지만 1~2초 뒤 룸 목록의 자동 선택 갱신이 "그 룸을 봤다"고 착각해 방금 올라간 알림을 지웠다. 화면에는 아무것도 남지 않아 "알림이 안 온다"로 보였다. Ping이 실제로 앞에 있을 때 일어난 선택 변경만 알림을 지우도록 했다.

## v0.3.70 macOS 수정

- 채팅 알림을 **쓸어 넘겨 지우면** 룸이 열리고 그 룸의 알림이 전부 정리되던 문제를 고쳤다. 지우는 동작과 누르는 동작을 구분하지 않았다.
- 설정의 "알림 소리"를 "없음"으로 해도 채팅 알림에서만 소리가 나던 문제를 고쳤다. 채팅 알림만 시스템 기본 소리로 고정돼 있었다.
- 채팅 알림을 띄울지 말지 판단한 근거를 진단 로그에 남긴다. 이 결정은 원격에서 볼 수 없어 원인 파악이 여러 번 어긋났다.

## v0.3.69 macOS 수정

- **룸 창을 열어둔 채 다른 앱을 쓰는 동안 채팅 알림이 사라지던 문제를 고쳤다.** 보고 있는 룸의 채팅은 알림을 띄우지 않는 게 맞지만, "보고 있다"를 창이 떠 있는지로만 판단했다. macOS에서 창은 다른 앱에 **완전히 가려져 있어도** 떠 있는 것으로 취급되므로, Ping 룸 창을 열어둔 채 브라우저를 보고 있으면 도착한 채팅이 알림도 뜨지 않고 휴대폰 푸시까지 억제됐다. 영상 핑은 이 판단을 타지 않아 정상적으로 떴고, 그래서 "영상은 뜨는데 텍스트는 안 뜬다"로 보였다. 이제 Ping이 실제로 앞에 있을 때만 보고 있는 것으로 본다.

## v0.3.68 macOS 수정

- **실시간 연결이 끊긴 뒤 다시 붙는다.** 0.3.67은 연결에는 성공했지만 한 시간을 못 버티고 끊긴 뒤 영구히 10초 폴링으로 돌아갔다. 원인 두 가지를 고쳤다.
  - Realtime에 넘기는 인증 토큰을 연결 시점에 복사해 두고 계속 그 값을 돌려줬다. access token은 1시간이면 만료되는데 Realtime은 이 값으로 주기적으로 재인증하므로, 만료 후에는 서버가 소켓을 닫고 재연결도 같은 만료 토큰을 써서 절대 복구되지 않았다. 이제 필요할 때마다 현재 토큰을 읽는다.
  - 재연결 판단이 클라이언트 **객체**의 존재만 봤다. 객체가 남아 있으면 "자동 복구되겠지" 하고 아무것도 하지 않았는데, 위 토큰 문제 때문에 자동 복구가 안 됐다. 이제 실제 소켓 상태를 보고, 죽어 있으면 60초 간격으로 다시 붙는다. 상태 감시가 조용한 종료를 놓친 경우도 같은 경로로 복구한다.

## v0.3.67 macOS 수정

- **실시간 연결이 실제 동작한다.** Realtime은 룸 상태·히스토리 갱신과 실행 중 자동 회신을 담당하고, 영상·채팅·초대 배너는 APNs로만 전달한다. 연결이 끊기면 polling으로 상태를 보완하지만 로컬 이벤트 배너를 만들지는 않는다.

## v0.3.66 macOS 수정

- 자동 재생된 영상을 **클릭하면 바로 사라진다.** 마우스를 올리면 닫기 버튼이 보인다. 0.3.65에서는 자동 재생 시 포커스를 넘기지 않으려고 창을 뒤쪽에 띄웠는데, 그 탓에 Esc·Enter·Space 같은 키가 앱에 전달되지 않고 클릭도 통과해서 10초 타임아웃 전까지 창을 지울 방법이 없었다. 자동 재생도 알림 클릭과 똑같이 앱을 앞으로 가져오도록 되돌려 Esc가 다시 동작한다.
- 핑이 도착한 직후 수신자가 그 룸을 열면 알림과 자동 재생이 통째로 취소되던 문제를 고쳤다. 룸을 여는 동작이 그 룸의 안 본 영상을 전부 "본 것"으로 표시하는데, 수신 확인이 10초 주기라 아직 뜨지도 않은 알림까지 같이 지워졌다. 이제 최근 60초 안에 도착한 영상은 룸을 열어도 알림 경로를 그대로 탄다.

## v0.3.65 macOS 수정

- 상대가 보낸 영상이 알림 클릭 없이 화면에 바로 뜬다. 발신자가 지정한 위치에 그대로 재생되고, 재생이 끝나면 10초 뒤 저절로 사라진다. APNs 배너를 클릭한 경우에도 같은 재생 경로를 사용하며, 앱을 켜기 전에 도착한 핑은 APNs 배너로만 알린다.
- 영상 수신 처리는 APNs 배너와 재생 준비를 분리해 느린 다운로드 한 건이 뒤따르는 알림을 막지 못한다.
- 채팅 실시간 연결을 10초마다 새로 맺으면서 이전 연결과 감시 작업이 정리되지 않고 쌓이던 누수를 고쳤다. 이 누수 때문에 진단 로그가 148만 건(470MB)까지 불어나 무료 플랜 데이터베이스 한도를 거의 채웠다. 같은 룸에 이미 붙어 있으면 다시 연결하지 않고, 연결을 교체할 때는 이전 것을 반드시 정리한다.
- 진단 로그에 전송 간격과 세션당 상한을 둬서 같은 종류의 폭주가 다시 일어나지 않게 했다.
...
- 이미 룸에서 조회한 수신 영상은 앱 재시작 후 다시 처리하지 않도록 polling 중복 원장과 서버 알림 RPC를 함께 보강했다.
...
- 오너 전용 인앱 다중 계정 전환 기능을 추가했다. 닉네임을 `영민`으로 설정하면 설정 → 일반에 "계정" 섹션이 나타나, 한 기기에서 여러 익명 계정을 보관·전환하고 전환 시 비활성 동안의 상태 변화(영상·초대·채팅)를 모아 볼 수 있다. 세션은 기기에 로컬 저장되어 타인에게 자격증명이 노출되지 않는다.
...
- Apple Developer Program 가입 후 릴리스 빌드를 ad-hoc 서명에서 **Developer ID Application 서명 + Apple 공증(notarization) + 스테이플**로 전환했다. 공증된 DMG는 Gatekeeper가 `accepted / Notarized Developer ID`로 판정한다.
- Developer ID designated requirement는 팀 + 번들 ID에 앵커링되어 이후 Developer ID 서명 업데이트의 TCC 권한 연속성을 지원한다. 이전 ad-hoc 릴리즈에서 Developer ID 릴리즈로 전환할 때는 Screen Recording 또는 카메라 권한을 다시 요청할 수 있으므로, 실제 Mac에서 이전 앱에 권한을 허용한 뒤 Sparkle 업데이트를 수행하는 테스트를 릴리즈 QA에 포함한다. 전환 시 권한이 유지된다고 보장하지 않는다.
...
- 릴리즈 앱의 designated requirement를 bundle id 기준으로 고정했고, 이후 Developer ID 서명 업데이트의 TCC 권한 연속성을 지원한다. 이전 ad-hoc 릴리즈에서의 전환은 Screen Recording·카메라 재승인 여부를 실제 Mac에서 확인해야 한다.
