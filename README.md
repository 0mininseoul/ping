...
1. `Ping-v0.3.64.dmg`를 더블클릭해 마운트한다.
...
- Option+L 프리뷰에서 Option+아래→위 스크롤: 프리뷰 창은 고정한 채 전송할 화면 영역을 1.0×~4.0×로 확대한다. Option+위→아래 스크롤로 축소한다.
- Option+L 프리뷰에서 Option+트랙패드 두 손가락 펼치기/오므리기: 커서 위치와 관계없이 캡처 영역을 확대/축소한다.
...
## v0.3.64 macOS/iOS 수정

- 리프레시 실패를 일시적(오프라인·408·429·5xx)과 영구적(거부된 토큰)으로 구분해, 일시적 실패는 재시도 사다리로 보내고 영구적 실패만 세션 만료로 처리한다. 그전에는 모든 실패가 세션 만료가 되면서 재시도 사다리가 도달 불가능한 코드였고, 로그인 직후 Wi-Fi가 붙기 전에 앱이 뜨면 "기존 Supabase 익명 세션을 복구할 수 없습니다" 모달이 떴다.
- 네트워크가 복구되면 백엔드 연결을 자동으로 다시 시도한다. 자동 시작(0.3.63)이 네트워크보다 먼저 앱을 띄우기 때문에, 그전에는 첫 실패 이후 앱을 껐다 켜기 전까지 복구되지 않았다.
- iOS: 룸 목록이 비어 있는 것과 서버에 연결하지 못한 것을 화면에서 구분하고, 세션이 만료됐으면 다시 연결하기 버튼을 제공한다. 대화 화면은 일시적 오류에 기존 메시지를 지우지 않고 배너로 알린다.
- iOS: 거부된 토큰으로 2초마다 무한 재시도하던 것을 멈추고, 복구 가능한 실패는 백오프로 간격을 늘린다.

## v0.3.63 macOS 수정

- **잠자기 후 앱이 꺼져 있던 문제를 해결했다.** 앱이 스스로 종료된 게 아니라, 디스크 공간을 회수하는 macOS 데몬 `com.apple.cache_delete`가 유휴·다크웨이크 구간에 Ping을 강제 종료하고 있었다. 이 종료는 크래시 리포트를 남기지 않아 흔적 없이 사라진 것처럼 보였다(진단 당시 24시간 동안 4회 종료됨).
- 앱 번들에 LaunchAgent를 넣고 launchd의 KeepAlive로 수명을 관리하도록 바꿨다. 비정상 종료되면 수십 초 안에 자동으로 다시 뜨고, 사용자가 직접 종료한 경우에는 되살아나지 않는다. 로그인 항목만으로는 이걸 막을 수 없다 — 로그인 항목은 로그인 시점에만 실행되지, 세션 도중 죽은 앱을 되살리지 않는다.
- 설정에 "로그인 시 자동 시작" 토글을 추가했다.
- 자동 시작 등록 과정에서 Ping이 두 번 실행될 수 있어, 나중에 뜬 프로세스가 스스로 물러나는 중복 실행 가드를 넣었다.
...
- macOS 발신 메시지에서 텍스트 버블과 링크 프리뷰 카드가 서로 다른 오른쪽 여백을 쓰던 문제를 고쳐 같은 기준선으로 정렬되도록 했다.
- 실제 룸 타임라인을 렌더링해 발신 텍스트 버블이 링크 프리뷰 카드와 같은 trailing inset 안에 남는지 검증하는 회귀 테스트를 추가했다.

## v0.3.56 macOS 수정

- macOS 발신 텍스트 버블의 원본 `NSTextView`가 SwiftUI 버블 밖으로 그리는 문제를 막기 위해 clipping NSView 컨테이너 안에서 렌더링하도록 바꿨다.
- 실제 `NSHostingView`에 긴 URL row를 렌더링해 모든 하위 AppKit view가 타임라인 폭 안에 남는지 검증하는 회귀 테스트를 추가했다.

## v0.3.55 macOS 수정

- macOS 발신 텍스트 버블의 `NSTextView`를 계산된 폭과 높이로 고정해 긴 URL이 버블 밖으로 그려지지 않게 했다.
- 타임라인 row wrapper를 full-width로 확정해 발신 메시지의 우측 정렬이 대화 영역 안에서만 계산되도록 했다.

## v0.3.54 macOS 수정

- 긴 링크가 포함된 발신 텍스트 버블이 오른쪽으로 밀리거나 잘려 보이지 않도록 macOS 버블 폭 계산을 보정했다.
- macOS 텍스트 버블의 URL 줄바꿈을 문자 단위로 처리해 긴 URL이 대화 column을 밀어내지 않게 했다.

## v0.3.53 macOS/iOS 수정

- 링크 프리뷰 카드의 OG 이미지 영역을 줄여 대화 흐름을 덜 밀어내도록 조정했다.
- macOS 발신 링크 카드에 오른쪽 여백을 추가해 창 가장자리로 붙거나 잘려 보이는 현상을 줄였다.
- 링크 메타데이터 fetch가 일시 실패해도 fallback 카드를 캐시에 고정하지 않고 다음 표시 때 다시 시도한다.

## v0.3.52 macOS/iOS 수정

- 링크 프리뷰 카드를 작은 썸네일 행이 아니라 OG 이미지가 크게 보이는 세로형 카드로 바꿨다.
- macOS와 iOS 모두 제목, 설명, 도메인을 이미지 아래에 배치해 메시지 앱 스타일의 rich preview에 가깝게 표시한다.

## v0.3.51 macOS/iOS 수정

- 일반 URL 링크 프리뷰가 페이지 메타 태그 순서에 흔들리지 않고 `og:image`와 `og:description`을 우선 사용하도록 보강했다.
- `https://gatitagachon.vercel.app/` 공유 메시지의 Korean OG 제목, 설명, 이미지 추출 계약을 macOS와 iOS 공통으로 추가했다.
