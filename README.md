...
- **핑을 실시간으로 받으면 자동으로 얼굴을 3초 녹화해 보낸 사람에게 되돌려 보낸다.** 모든 macOS 사용자에게 항상 적용되며 설정에서 끌 수 없고, 관련 선택 UI도 표시하지 않는다. 녹화하는 동안 화면 우상단에 표시가 뜬다(카메라 화면은 띄우지 않는다). 자동 회신으로 온 핑에는 다시 자동 회신하지 않고, 맥이 꺼져 있었거나 도착한 지 1분이 지난 핑, 거울을 쓰는 중이면 건너뛴다.
...
- 앱 번들의 LaunchAgent와 launchd KeepAlive가 실제 실행 중인 Ping 프로세스를 소유한다. 잠자기·메모리 압박·디스크 압박·크래시 중 비정상 종료되면 자동으로 다시 뜨고, 사용자가 직접 종료한 경우에는 현재 로그인 세션에서 되살아나지 않는다. 리소스 압박에 의한 종료 자체를 막는 것이 아니라 종료 뒤 자동 복구하는 방식이다.
- 설정에 "로그인 시 자동 시작" 토글을 추가했다.
- 자동 시작 등록 과정에서 Ping이 두 번 실행되면 launchd가 띄운 프로세스가 기존 수동 실행 프로세스를 교체한다. 수동 재실행과 Sparkle 업데이트 후 재실행 시에는 등록을 현재 앱 번들로 갱신한 뒤 launchd 관리 프로세스로 소유권을 넘긴다.
