...
TestFlight에 최신 빌드 **build 17**(버전 0.1.1) 업로드 완료. 아래는 App Store Connect(ASC) 웹에서 직접 해야 하는 액션이다 (대부분 웹 UI라 CLI로 대신 못 함).
...
- **빌드 선택**: 버전(0.1.1) → "빌드" 섹션에서 **build 17** 선택. (TestFlight 처리 완료 상태여야 함)
...
3. ASC에서 2~8번 메타데이터/Privacy/연령/리뷰노트 입력 + build 17 선택.
4. **Submit for Review**.
